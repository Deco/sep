    ...
    while (running){
        ...
        switch(id) {
            ...
            case SENSOR_DATA: {
                ...
                newReading.time = 0;     // set time = to current time
                float *imgDataPtr = (float*)newReading.img.data;
                float *tempPtr = (float*)buff;

                // memcpy(imgDataPtr, buff, sizeof(float)*64);
                for(int x = 0; x < 16; x++) {
                    for(int y = 0; y < 4; y++) {
                        if((15-x) == 11 and (3-y) == 1) {
                            imgDataPtr[y*16+x] = -99999; // dead pixel.....
                        } else {
                            imgDataPtr[y*16+x] = tempPtr[(15-x)*4+(3-y)];
                        }
                    }
                }
 ...
 *              the queue. 
 *      timeRef: double passed by reference to return the time the reading at the
